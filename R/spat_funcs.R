...
dens.plot<-function(MCMC,xaxt=TRUE){
	...
	if(xaxt==FALSE)qplot(Value,data=Plot.df,geom="density")+scale_y_continuous("Density")+opts(axis.text.x=theme_blank(),axis.text.y=theme_blank())+facet_wrap(~type,scales="free")
	else qplot(Value,data=Plot.df,geom="density")+scale_y_continuous("Density")+opts(axis.text.y=theme_blank())+facet_wrap(~type,scales="free")
}
